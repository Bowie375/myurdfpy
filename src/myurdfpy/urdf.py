...
class URDF:
    ...
    def split_along_joints(self, joint_type="floating", **kwargs):
        ...
        Returns:
            list[(np.ndarray, myurdfpy.URDF)]: A list of tuples (np.ndarray, myurdfpy.URDF) where each homogeneous 4x4 matrix describes the root transformation of the respective URDF model w.r.t. the original URDF.
        """
        root_urdf = URDF(
        ...
        for joint_name in joint_names:
            ...
            for link in new_robot.links:
                root_urdf.robot.links.remove(root_urdf.link_map[link.name])
    ...
    def apply_visual_color(
        ...
        """
        if isinstance(visual, Collision) or visual.material is None:
            return

        if visual.material.color is not None:
            color = visual.material.color
        elif visual.material.name is not None and visual.material.name in self._material_map:
            color = self._material_map[visual.material.name].color
        else:
            return
    ...
    def _geometry2trimeshscene(
        ...
        Args:
            v (Visual or Collision): A visual or collision object.
        ...
        if geometry.box is not None:
            ...
            if os.path.isfile(new_filename):
                if force_mesh:
                    ...
                    self.apply_visual_color(new_g, v)
                    new_s = trimesh.Scene()
                    ...
                    for i, (_, geom) in enumerate(new_s.geometry.items()):
                        ...
                        self.apply_visual_color(geom, v)
    ...
    def _add_geometries_to_scene(
        self,
        s: trimesh.Scene,
        geometries: List[Collision] | List[Visual],
        link_name: str,
        load_geometry: bool,
        force_mesh: bool,
        ...
        for i, v in enumerate(geometries):
    ...
    def _create_scene(
        ...
        s: trimesh.Scene = trimesh.scene.Scene(base_frame=self._base_link)
        ...
        for link in self.robot.links:
            if link.name not in s.graph.nodes and link.name != s.graph.base_frame:
                _logger.warning(
                    f"{link.name} not connected via joints. Will add link to base frame."
                )
                s.graph.update(frame_from=s.graph.base_frame, frame_to=link.name)

            meshes = link.collisions if use_collision_geometry else link.visuals
